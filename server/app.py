...
TASKS = {
    "easy": {
        ...
        "patches": [
            ...
            {"id":"p3","type":"code","payload":"r = redis.Redis(host=localhost, port=6379)","token_cost":10,"is_critical":False},
    ...
    "medium": {
        ...
        "patches": [
            ...
            {"id":"p8","type":"equation","payload":"Cache hit ratio target >= 0.85","token_cost":11,"is_critical":False},
    ...
    "hard": {
        ...
        "patches": [
            ...
            {"id":"p3","type":"code","payload":"session = ort.InferenceSession(model.onnx)","token_cost":32,"is_critical":False},
            {"id":"p4","type":"constraint","payload":"Model accuracy must stay above 94.5 percent F1","token_cost":15,"is_critical":True},
            {"id":"p5","type":"decision","payload":"Feature store uses Feast with Redis online store","token_cost":12,"is_critical":False},
            {"id":"p6","type":"equation","payload":"Drift threshold: KL divergence > 0.1","token_cost":18,"is_critical":False},
            {"id":"p7","type":"structure","payload":"Pipeline: ingest preprocess extract infer postprocess","token_cost":10,"is_critical":False},
            {"id":"p8","type":"entity","payload":"Training data cannot include PII without consent","token_cost":14,"is_critical":True},
            {"id":"p9","type":"equation","payload":"Model v2.3.1 is current production checkpoint","token_cost":13,"is_critical":False},
            {"id":"p10","type":"decision","payload":"Shadow deployment for AB testing new models","token_cost":14,"is_critical":True},
            {"id":"p11","type":"constraint","payload":"model.run(None, input_dict)","token_cost":21,"is_critical":False},
            ...
            {"id":"p14","type":"entity","payload":"India managed by ML platform team on Slack","token_cost":10,"is_critical":False},
            {"id":"p15","type":"decision","payload":"Canary rollout at 5 percent before full promotion","token_cost":13,"is_critical":False},
        ]
    }
}

state = {"task_id":"easy","patches":[],"token_budget":0,"turn":0,"done":False}
...
def make_obs(task_id, patches, budget, turn):
    return Observation(patches=[Patch(**p) for p in patches],
        token_budget=budget-sum([p["token_cost"] for p in patches]),
...
def reset(task_id: str = Query(default="easy")):
    ...
    state.update({"task_id":task_id,"patches":task["patches"],"token_budget":task["token_budget"],"turn":0,"done":False})
    return ResetResult(observation=make_obs(task_id, task["patches"], task["token_budget"], 0))

@app.post("/step", response_model=StepResult)
def step(action: Action):
    if state["done"]:
        return StepResult(observation=make_obs(state["task_id"],state["patches"],
                _state["token_budget"],state["turn"]), reward=0.0, done=True,
                info={"message":"already done"})
    state["turn"] += 1
    state["done"] = True
    reward, crr, trr, kt = compute_reward(action.keep_patch_ids, state["patches"], state["token_budget"])
    return StepResult(
        observation=make_obs(state["task_id"],state["patches"],state["token_budget"],state["turn"]),
        reward=reward, done=True,
        info={"crr":round(crr,4),"trr":round(trr,4),"kept_tokens":kt,"budget":state["token_budget"]})
